...
package com.aredondocharro.ClothingStore.identity.adapter.out.jwt;
...
public class JwtTokenGeneratorAdapter implements TokenGeneratorPort {
    ...
    public String generateAccessToken(User u) {
        String token = JWT.create()
                ...
                .withArrayClaim("roles", u.getRoles().toArray(new String[0]))
                .withClaim("type", "access")
                .withExpiresAt(Date.from(Instant.now().plusSeconds(accessSeconds)))
                .sign(alg);
        log.debug("Access token generated for userId={}", u.getId());
        return token;
    }
    ...
    public String generateRefreshToken(User u) {
        String token = JWT.create()
                ...
                .sign(alg);
        log.debug("Refresh token generated for userId={}", u.getId());
        return token;
    }
    ...
    public String generateVerificationToken(User u) {
        String token = JWT.create()
                ...
                .sign(alg);
        log.debug("Verification token generated for userId={}", u.getId());
        return token;
    }
}
