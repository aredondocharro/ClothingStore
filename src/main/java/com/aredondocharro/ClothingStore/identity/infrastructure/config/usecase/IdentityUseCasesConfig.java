...
public class IdentityUseCasesConfig {
    ...
    public RegisterUserUseCase registerUserUseCase(
            ...
            PasswordHasherPort hasher, PasswordPolicyPort passwordPolicy,
            Clock clock, EventBusPort eventBus) {
        return new RegisterUserService(loadUserPort, saveUserPort, hasher, passwordPolicy, clock, eventBus);
    }

    @Bean
    public LoginUseCase loginUseCase(
            LoadUserPort loadUserPort, PasswordHasherPort hasher,
            TokenGeneratorPort tokens, RefreshTokenStorePort store,
            RefreshTokenVerifierPort refreshVerifier, Clock clock) {
    ...
    public VerifyEmailUseCase verifyEmailUseCase(
            VerificationTokenPort verifierToken, LoadUserPort loadUserPort,
            SaveUserPort saveUserPort, TokenGeneratorPort tokens,
            RefreshTokenStorePort store, RefreshTokenVerifierPort refreshVerifier, Clock clock) {
    ...
    public RefreshAccessTokenUseCase refreshAccessTokenUseCase(
            RefreshTokenVerifierPort refreshVerifier, RefreshTokenStorePort store,
            LoadUserPort loadUserPort, TokenGeneratorPort tokens) {
        return new RefreshAccessTokenService(refreshVerifier, store, loadUserPort, tokens);
    }

    @Bean
    public LogoutUseCase logoutUseCase(RefreshTokenVerifierPort refreshVerifier, RefreshTokenStorePort store) {
        return new LogoutService(refreshVerifier, store);
    }

    @Bean
    public DeleteUserUseCase deleteUserUseCase(UserAdminRepositoryPort userAdminRepository, SessionManagerPort sessionManager) {
        return new DeleteUserService(userAdminRepository, sessionManager);
    }

    @Bean
    public UpdateUserRolesUseCase updateUserRolesUseCase(UserAdminRepositoryPort userAdminRepository) {
        return new UpdateUserRolesService(userAdminRepository);
    }

    @Bean
    public RequestPasswordResetUseCase requestPasswordResetUseCase(
            UserRepositoryPort users, PasswordResetTokenRepositoryPort tokens, EventBusPort eventBus,
            @Value("${app.reset.baseUrl}") String baseUrl, Clock clock, @Value("${app.reset.ttl}") Duration ttl) {
        return new RequestPasswordResetService(users, tokens, eventBus, baseUrl, clock, ttl);
    }

    @Bean
    public ResetPasswordUseCase resetPasswordUseCase(
            PasswordResetTokenRepositoryPort tokens, PasswordPolicyPort passwordPolicy,
            UserRepositoryPort users, PasswordHasherPort passwordHasher,
            SessionManagerPort sessions, Clock clock) {
    ...
    public ChangePasswordUseCase changePasswordUseCase(
            UserRepositoryPort users, PasswordHasherPort passwordHasher,
            PasswordPolicyPort passwordPolicy, SessionManagerPort sessions) {
